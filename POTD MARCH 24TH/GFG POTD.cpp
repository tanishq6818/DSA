...
stack<int> insertAtBottom(stack<int> st, int x) {
    // Base case: if the stack is empty, just push the element x and return
    if (st.empty()) {
        st.push(x);
        return st;
    }

    // Pop all elements from the stack and store them in the function call stack
    // Then insert the element x at the bottom
    int temp = st.top();
    st.pop();
