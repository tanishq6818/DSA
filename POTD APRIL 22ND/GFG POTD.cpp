...
int minRow(int n, int m, vector<vector<int>> a) {
    int minRowIndex = 0; // Initialize the index of the row with minimum 1's to the first row
    int minOnes = m; // Initialize the minimum number of 1's to the maximum possible value
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            if (a[i][j] == 1) // If the element is 1, increment onesCount
                ++onesCount;
        }
        
        // Update minRowIndex if the current row has fewer 1's
    ...
    return minRowIndex + 1; // Return the minimum index (1-based)
}
