...
int maxSumIS(int arr[], int n)
{
    ...
    // Initialize the dp array with the input array values
    for (int i = 0; i < n; i++) {
        dp[i] = arr[i];
    }
...
You can call this function with the given arr and n to get the maximum sum of the strictly increasing subsequence. This code has a time complexity of O(N^2) and uses an auxiliary space of O(N).
