...
void dfs(unordered_map<string, vector<string>>& graph, string email, set<string>& visited, vector<string>& component) {
...
vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
    ...
    // Construct the graph and map email to name
    ...
    for (const auto& account : accounts) {
        ...
        for (int i = 1; i < account.size(); ++i) {
            if (visited.find(account[i]) == visited.end()) {
                ...
                dfs(graph, account[i], visited, component);
