...
private:
    int countPaths(Node *node, int currentSum, int target, unordered_map<int, int> &prefixSumCount)
    {
        if (!node)
            return 0;

        currentSum += node->data;
        int paths = prefixSumCount[currentSum - target];
        ...
        paths += countPaths(node->left, currentSum, target, prefixSumCount);
        paths += countPaths(node->right, currentSum, target, prefixSumCount);
