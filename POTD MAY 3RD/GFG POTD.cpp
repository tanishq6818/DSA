...
vector<int> Kdistance(struct Node *root, int k) {
    ...
    function<void(Node*, int)> helper = [&](Node* node, int level) {
      if (node == nullptr) return;

      // If current level is same as k, add node data to result
      if (level == k) {
        result.push_back(node->data);
        return;
      }

      // Recursively traverse left and right subtrees with incremented level
