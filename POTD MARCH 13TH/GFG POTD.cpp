...
vector<int> matrixDiagonally(vector<vector<int>>&arr) { map<int,vector<int>>m; for(int i=0;i<arr.size();i++) { for(int j=0;j<arr.size();j++) { m[i+j].push_back(arr[i][j]); } } vector<int>v; int k=0; for(auto i:m) { if(k%2==0 and k>0) { reverse(i.second.begin(),i.second.end()); } for(auto ele:i.second) { v.push_back(ele); } k++; } return v; //Your code here }
}
