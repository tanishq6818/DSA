...
public:
    int floor(Node* root, int x) {
        int floorValue = -1;  // Initialize with -1 as mentioned in the note
        while (root) {
            if (root->data == x) {
                return root->data;  // Found an exact match
            } else if (root->data > x) {
                root = root->left;  // Move to the left subtree
            } else {
                floorValue = root->data;  // Update the floor value and move to the right subtree
                root = root->right;
            }
        }
        return floorValue;
