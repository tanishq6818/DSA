...
vector<int> recamanSequence(int n) {
    vector<int> result;
    unordered_set<int> seen; // To keep track of numbers already in the sequence
    result.push_back(0);
    seen.insert(0);

    for (int i = 1; i < n; i++) {
        int current = result[i - 1] - i;

        // If the number is positive and not already in the sequence, use it
        if (current > 0 && seen.find(current) == seen.end()) {
            result.push_back(current);
            seen.insert(current);
        } else {
            // Otherwise, use the alternative value
            current = result[i - 1] + i;
            result.push_back(current);
            seen.insert(current);
        }
    }
