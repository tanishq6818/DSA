...
0 ≤ l[i], r[i] ≤ 106
  ...
  int maxOccured(int n, int l[], int r[], int maxx) {
    // Create a frequency array with size maxx + 2
    vector<int> freq(maxx + 2, 0);

    // Update the frequency array based on the given ranges
    for (int i = 0; i < n; i++) {
        freq[l[i]] += 1;
        freq[r[i] + 1] -= 1;
    }

    // Compute the prefix sum to find the actual number of active ranges at each point
    int max_freq = freq[0];
