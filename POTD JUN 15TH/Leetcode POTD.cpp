...
0 <= capital[i] <= 109
  ...
  int findMaximizedCapital(int k, int w, vector<int>& profits, vector<int>& capital) {
    // Create a vector of pairs (capital, profit) for each project
    ...
    // Max-heap to store the profits of projects that we can afford to start
    ...
    while (k--) {
        ...
        // If we can't start any new project, break out
        if (maxHeap.empty()) {
            break;
        }
